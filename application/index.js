...
if (!status.notKaiOS) {
  const scripts = ["./assets/js/kaiads.v5.min.js"];
...
let sendMessage = (
  ...
  if (type === "audio") {
    ...
    if (typeof msg.arrayBuffer === "function") {
      // Moderne Browser
      msg
        .arrayBuffer()
        .then(handleBuffer)
        .catch((error) => {
          console.error(
            "Error converting Blob to ArrayBuffer (modern):",
            error
          );
        });
    } else {
      // KaiOS2
      const reader = new FileReader();
      reader.onload = () => handleBuffer(reader.result);
      reader.onerror = (error) => {
        console.error(
          "Error converting Blob to ArrayBuffer (fallback):",
          error
        );
      };
...
let peer_is_online = async function () {
  ...
  try {
    ...
    for (let i = 0; i < addressbook.length; i++) {
      ...
      //when peer is not connected try to connect
      try {
        let user_meta = {
        ...
        console.log("peer is online error 1: " + error);
        entry.live = false;
        m.redraw();
      }
    }
    ...
    console.log("peer is online error 2: " + error);
  }
};
                      ...
                      m(
                        ...
                        [
                          m("div", { class: "icon col-xs-2 col-md-2" }, [
                            m(
                              "div",
                              { class: "online-indicator user-offline" },
                              ""
                            ),
                            m("img", {
                              class: "aavatar",
                          ...
                          m("div", { class: "inner col-xs-10 col-md-10" }, [
                            ...
                            m("div", { class: "row between-md between-xs" }, [
        ...
        oninit: () => {
          audioRecorder.startRecording().then(() => {
            audio_recorder_time = setInterval(() => {
              ...
              let t = dayjs(audioRecorderDuration * 1000).format("mm:ss");
              top_bar("", t, "");
            }, 1000);
